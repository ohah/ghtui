...
pub async fn execute(client: &GithubClient, cmd: Command) -> Message {
    match cmd {
        ...
        Command::FetchPrDetail(repo, number) => {
            match client.get_pull_detail(&repo, number).await {
                Ok(detail) => Message::PrDetailLoaded(Box::new(detail)),
                Err(e) => Message::Error(e.into()),
            }
        }
        Command::FetchPrDiff(repo, number) => {
            match client.get_pull_diff(&repo, number).await {
        ...
        Command::ClosePr(repo, number) => {
            match client.close_pull(&repo, number).await {
                Ok(()) => Message::PrClosed(number),
                Err(e) => Message::Error(e.into()),
            }
        }
        Command::ReopenPr(repo, number) => {
            match client.reopen_pull(&repo, number).await {
                Ok(()) => Message::PrReopened(number),
                Err(e) => Message::Error(e.into()),
            }
        }
        Command::CreatePr(repo, input) => {
            match client.create_pull(&repo, &input).await {
        ...
        Command::CloseIssue(repo, number) => {
            match client.close_issue(&repo, number).await {
                Ok(()) => Message::IssueClosed(number),
                Err(e) => Message::Error(e.into()),
            }
        }
        Command::ReopenIssue(repo, number) => {
            match client.reopen_issue(&repo, number).await {
                Ok(()) => Message::IssueReopened(number),
                Err(e) => Message::Error(e.into()),
            }
        }
        Command::CreateIssue(repo, input) => {
            match client.create_issue(&repo, &input).await {
        ...
        Command::FetchRunDetail(repo, run_id) => {
            match client.get_run_detail(&repo, run_id).await {
        ...
        Command::CancelRun(repo, run_id) => {
            match client.cancel_run(&repo, run_id).await {
                Ok(()) => Message::RunCancelled(run_id),
                Err(e) => Message::Error(e.into()),
            }
        }
        Command::RerunRun(repo, run_id) => {
            match client.rerun_run(&repo, run_id).await {
        ...
        // Notifications
        Command::FetchNotifications(filters) => {
            match client.list_notifications(&filters).await {
        ...
        // Search
        Command::Search(query, kind, page) => {
            match client.search(&query, kind, page).await {
